...
import { imageSizesService, ImageSizeFormData } from "@/lib/services/image-sizes.service";
import { ArrowLeft, Save } from "lucide-react";

export default function NewImageSizePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await imageSizesService.createImageSize(formData);
      router.push("/design/image-sizes");
    } catch (error: any) {
      console.error("Error creating image size:", error);
      alert(error.response?.data?.error?.message || "Неуспешно създаване");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-title-md font-bold text-gray-900 dark:text-white">
          Нов размер на изображение
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Създайте нов template за размер на изображение
        </p>
      </div>
